 ...
 * По умолчанию размер окна браузера при remote запуске равен 1280x800
...
public class SelenoidDriverProvider implements WebDriverProvider {
    ...
    public WebDriver createDriver(DesiredCapabilities capabilities) {
        String expectedBrowser = loadSystemPropertyOrDefault(BROWSER, CHROME_DRIVER);
        String remoteUrl = loadSystemPropertyOrDefault(REMOTE_URL, "local");
        BlackList blackList = new BlackList();

        if (FIREFOX_DRIVER.equalsIgnoreCase(expectedBrowser)) {
    ...
    private WebDriver getRemoteDriver(DesiredCapabilities capabilities, String remoteUrl) {
        ...
        Integer browserWidth = loadSystemPropertyOrDefault(WINDOW_WIDTH, 1280);
        Integer browserHeight = loadSystemPropertyOrDefault(WINDOW_HEIGHT, 800);
