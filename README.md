...
Для этого была реализована возможность создания файла без расширения ```blacklist``` , хранящего указанные пользователем ресурсы по пути:
...
Формат записи - каждый новый ресурс с новой строки, например:
     ...
     http://google.com/ 200
При необходимости можно указывать возвращаемый статус код через пробел, по умолчанию будет присвоен ```404```
